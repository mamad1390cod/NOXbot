...
@router.callback_query(F.data.startswith("admin:roles:perm:"))
async def cb_toggle_perm(callback: CallbackQuery, uow, user: User) -> None:
    parts = callback.data.split(":")
    # admin:roles:perm:<role_id>:<perm_code>   (perm_code = short stable code)
    if len(parts) < 5:
        await callback.answer("دسترسی نامعتبر", show_alert=True)
        return
    role_id = parts[3]
    perm_token = parts[4]
    perm_value = permission_codec().decode(perm_token) or perm_token
    try:
...
# --- Role change -------------------------------------------------------- #
@router.callback_query(F.data.startswith("admin:roles:changerole:"))
async def cb_change_role(callback: CallbackQuery, state: FSMContext) -> None:
    parts = callback.data.split(":")
    if len(parts) < 4:
        await callback.answer("دسترسی نامعتبر", show_alert=True)
        return
    user_id = parts[3]  # target user's id
...
@router.message(AdminRolesStates.waiting_role_pick)
async def _hint_adminrolesstates_waiting_role_pick(message: Message) -> None:
    await message.answer("برای انتخاب نقش، روی یکی از دکمه‌های بالا بزنید 👆")
