...
async def cb_payment_request_again(callback: CallbackQuery, uow, user: User) -> None:
    ...
    await uow.flush()
    if payment:
        # Re-read with relations loaded: touching payment.user on the updated
        # instance triggered a lazy load in async context (MissingGreenlet).
        payment = await uow.payments.get_payment_with_details(payment_id)
