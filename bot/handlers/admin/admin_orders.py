...
from bot.states import AdminOrderStates
...
async def cb_aorder_fs_payment(callback: CallbackQuery, uow, user: User) -> None:
    parts = callback.data.split(":", 2)
    if len(parts) < 3:
        await callback.answer("وضعیت پرداخت نامعتبر", show_alert=True)
        return
    val = parts[2]
    f = _ACTIVE_FILTERS.setdefault(user.telegram_id, {})
...
async def cb_aorder_refund(callback: CallbackQuery, uow, user: User) -> None:
    ...
    if not order:
        return
    await _transition(callback, uow, user, order, OrderStatus.REFUNDED, note="بازگشت وجه")
