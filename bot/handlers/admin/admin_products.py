...
async def cb_product_detail(callback: CallbackQuery, uow, user: User) -> None:
    ...
    product = await ps.get_product(product_id)
    if not product:
        await callback.answer("محصول یافت نشد", show_alert=True)
        return
...
async def cb_move_product(callback: CallbackQuery, uow, user: User) -> None:
    product_id = callback.data.split(":", 2)[2]
    cats = await CategoryService(uow).get_visible_categories("product")
    keyboard = []
    for c in cats:
        keyboard.append([types.InlineKeyboardButton(text=c.name, callback_data=cb("aprod:move_to", product_id, c.id))])
    keyboard.append([back_button("admin:products")])
    await safe_edit_text(callback, "↪️ انتخاب دسته جدید:", reply_markup=types.InlineKeyboardMarkup(inline_keyboard=keyboard))
    await callback.answer()


@router.callback_query(F.data.startswith("aprod:move_to:"))
async def do_move_product(callback: CallbackQuery, uow, user: User) -> None:
...
async def _hint_productstates_waiting_category(message: Message) -> None:
