...
class User(Base, UUIDMixin, TimestampMixin):
    ...
    referral_code: Mapped[str] = mapped_column(
        String(20),
        # Model-level default so *any* code path that creates a User (seeds,
        # imports, tests) satisfies the NOT NULL constraint; UserService still
        # passes its own collision-checked code.
        default=lambda: secrets.token_urlsafe(8)[:10].upper(),
        unique=True,
        nullable=False,
        index=True,
    )
    ...
    # NOTE: handlers/keyboards format users in messages; without these two
    # properties every "{user.display_name}" / "{user.full_name}" interpolation
    # raised AttributeError at runtime (broken admin buttons).
    @property
    def full_name(self) -> str:
        """First + last name, or an empty string when Telegram sent neither."""
        parts = [p for p in (self.first_name, self.last_name) if p]
        return " ".join(parts)

    @property
    def display_name(self) -> str:
        """Best human label for this user: name → @username → numeric id."""
        return self.full_name or (f"@{self.username}" if self.username else str(self.telegram_id))

    @property
    def mention(self) -> str:
        """HTML mention link (safe for parse_mode=HTML messages)."""
