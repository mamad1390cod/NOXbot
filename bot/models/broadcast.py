...
class Broadcast(Base, UUIDMixin, TimestampMixin):
    ...
    status: Mapped[BroadcastStatus] = mapped_column(
        # Declared as a plain String(30) before, so rows came back as raw ``str``
        # and every ``broadcast.status.value`` raised AttributeError. Stored as
        # the enum *value* ("draft", "pending", ...) which is exactly what the
        # old column already contains — no data migration needed.
        Enum(
            BroadcastStatus,
            native_enum=False,
            length=30,
